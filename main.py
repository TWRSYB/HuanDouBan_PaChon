...
from Config.Config import JSON_DATA_ACTOR, PIC_DIR_ACTOR_AVATAR
from Config.ReqConfig import URL_HOST_API, API_PATH_ACTOR_LIST, API_PATH_ACTOR_DETAIL
...
def get_actor_list_page(cid, page_num) -> List[ActorVo]:
    ...
    dict_res = req_util.try_ajax_get_times(url=f"{URL_HOST_API}{API_PATH_ACTOR_LIST}", params=params,
    ...
    if dict_res:
        ...
        com_log.error(f"获取演员列表失败: 第{page_num}页")
    return actor_list
...
def save_actor(actor_id, cid) -> ActorVo:
    ...
    dict_res = req_util.try_ajax_post_times(f'{URL_HOST_API}{API_PATH_ACTOR_DETAIL}', data=data,
    ...
    if dict_res:
        ...
        com_log.error(f"获取演员信息失败, 演员ID: {actor_id}")
    return actor_vo
